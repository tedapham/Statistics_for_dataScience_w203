...
hist(df_naomit$avg_educ
     , breaks = seq(-0.5, 18, 1)
     , col = "blue"
     , main = "Mother's and Father's Education"
...
```
High correlation between Mother's and Father's ages that could lead to multi-colinearity between the two variables.
...
stargazer::stargazer(model1_v5, model2, model3
                    ...
                    , covariate.labels = c("Prenatal Care (Month)", "Prenatal Visits"
                                          ...
                                          , "Father's Ages", "Parents' Education"
...
We observed multiple instances with statiscal significance:
...
4/ With this dataset, there are almost 1600 mothers who do not drink and only 16 who do, soeven though there is a difference of about 80 grams of birthweight for the baby between the two groups
...
4/ Prenatal care visit and cigarette usage during the pregancy can affect newborn weight

# 6. Discussions
We cannot draw casuality from the model for two reasons:
1/ The fits are not ideal, at on 2.29% 
2  There's too many other ommitted variables that could be causing bias. For examples
   a/ Parents' income
